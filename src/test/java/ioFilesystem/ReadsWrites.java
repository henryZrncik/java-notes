...
public class ReadsWrites {
    @Test void readFile() throws IOException {  // Read line by line
        Path filePath = Paths.get("src/data/specialCharactersText.txt");
        BufferedReader bufferedReader = Files.newBufferedReader(filePath);
        String line;
        while ((line = bufferedReader.readLine()) != null) System.out.println(line);
    }
    @Test void writeText() throws IOException { // write what i want
        BufferedWriter bf = new BufferedWriter( new FileWriter("src/data/specialCharactersText.txt")); // overwrite
        bf.write("log1\n"); // append
        bf.write("log2\n"); //append
        bf.close();
    }
    @Test void saveObjects() throws IOException, ClassNotFoundException {
        var out = new ObjectOutputStream(new FileOutputStream("src/data/Object.txt"));
    ...
    @Test void loadObjects() throws IOException, ClassNotFoundException {
        var in = new ObjectInputStream(new FileInputStream("src/data/Object.txt"));
